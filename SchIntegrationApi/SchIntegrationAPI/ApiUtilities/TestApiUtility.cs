...
namespace SchIntegrationAPI.ApiUtilities
{
    public class TestApiUtility
    {
        ...
        private Logger Logger { get; } = LogManager.GetLogger("logfile");
        private Logger LoggerAttachment { get; } = LogManager.GetLogger("logfileattachment");
        private Logger LoggerJson { get; } = LogManager.GetLogger("logfilejson");

       // private EmailNotificationUtility _emailNotificationUtility = null;

        public TestApiUtility(AppConfigs appConfigs)
        {
        ...
        public JObject ApiCallForChildCollection(string webapiLink)
        {
            ...
            try
            {
                ...
                if (response.Content != null && response.IsSuccessStatusCode)
                {
                    ...
                    LoggerJson.Info($"child api link is- {webapiLink} \n Get Child request Json=>{json}");
            ...
            try
            {
                if (assignment_group != null)
                {
                    ...
                    if (response.Content != null && response.IsSuccessStatusCode)
                    {
                        var json = response.Content.ReadAsStringAsync().Result;
                        json = json.Contains("\n") ? json.Replace("\n", "").ToString() : json;
                        LoggerJson.Info($"Get Request Json=>{json}");
                        ...
                        foreach (JProperty p in o.Properties())
                        {
                            ...
                            foreach (JObject item in value.Where(s=>s["state"].ToString() != "3"))
                            {
        ...
        public List<SchApi> GetMainRequestByAssignGroup(string requestType, string assignment_group)
        {
            List<SchApi> accruentApisList = new List<SchApi>();
        ...
        public string GetMainRequestJsonByAssignGroup(string requestType, string assignment_group)
        {
            // List<AccruentApi> accruentApisList = new List<AccruentApi>();
            string json = "";
            try
            {
                if (assignment_group != null)
