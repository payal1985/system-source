    ...
    public class Aws3Service
    {
        ...
        public async Task UploadFilesToAWS(List<Tuple<int, string>> data)
        {
            if (data.Count > 0)
            {
                ...
                //    {
                //        Task.Run(() => UploadFileAsync(item.Item2, item.Item1));
                //        //await UploadFileAsync(item.Item2, item.Item1);
                //    }
                //}
        ...
        public async Task<bool> UploadFileAsync(string filePath, int clientId)
        {
            try
            {
                ...
                //    var key = AwsClientConfiguration.Folder + clientId + "/" + fileName;
                //    var fileTransferUtility = new TransferUtility(AmazonS3Client);
                //    await fileTransferUtility.UploadAsync(fileToUpload, AwsClientConfiguration.BucketName, key);
                //}
        ...
        {
            BucketName = configuration.GetValue<string>("ExternalSystem:AwsSettings:BuketName"),
